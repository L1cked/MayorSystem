...
class SystemCommands(private val ctx: CommandContext) {
    ...
    private val rewardModeSuggestions = SuggestionProvider.suggestingStrings<CommandSender>(
        DisplayRewardMode.entries.map { it.name }
    )
    private val booleanSuggestions = SuggestionProvider.suggestingStrings<CommandSender>("true", "false")
    private val rewardTargetTypeSuggestions = SuggestionProvider.blockingStrings<CommandSender> { context, _ ->
        if (!canSuggestReward(context.sender(), edit = true)) emptyList() else listOf("track", "group", "user")
    }
    private val rewardViewTargetTypeSuggestions = SuggestionProvider.blockingStrings<CommandSender> { context, _ ->
        if (!canSuggestReward(context.sender(), edit = false)) emptyList() else listOf("track", "group", "user")
    }
    private val rewardMenuSuggestions = SuggestionProvider.blockingStrings<CommandSender> { context, _ ->
        if (!canSuggestReward(context.sender(), edit = false)) {
            emptyList()
        } else {
            listOf("main", "tracks", "groups", "users", "tag", "tag_icon", "health")
        }
    }
    private val rewardTargetSuggestions = SuggestionProvider.blockingStrings<CommandSender> { context, _ ->
        if (!canSuggestReward(context.sender(), edit = false)) return@blockingStrings emptyList()
    ...
    private val tagIdSuggestions = SuggestionProvider.blockingStrings<CommandSender> { context, _ ->
        if (!canSuggestReward(context.sender(), edit = true)) {
            emptyList()
        } else {
            (
    ...
    private val itemMaterialSuggestions = SuggestionProvider.blockingStrings<CommandSender> { context, _ ->
        if (!canSuggestReward(context.sender(), edit = true)) {
            emptyList()
        } else {
            Material.entries
    ...
    fun register() {
        ...
        cm.command(
            ctx.rootCommandBuilder()
                ...
                .handler { command ->
                    val admin: Player = command.sender()
    ...
    private fun registerDisplayRewardCommands() {
        ...
        cm.command(
            ctx.rootCommandBuilder()
                ...
                .permission(editPerm)
                .senderType(Player::class.java)
                .required("value", stringParser(), SuggestionProvider.blockingStrings<CommandSender> { context, _ ->
                    if (!canSuggestReward(context.sender(), edit = true)) emptyList() else luckPermsGroupNames()
                })
                .handler { command ->
                    val admin: Player = command.sender()
                    val value = command.get<String>("value").trim()
    ...
    private fun registerTagRewardCommands(editPerm: Permission) {
        ...
        cm.command(
            ctx.rootCommandBuilder()
                ...
                .permission(editPerm)
                .senderType(Player::class.java)
                .required("value", stringParser(), SuggestionProvider.suggestingStrings<CommandSender>("clear", "reset", "0"))
                .handler { command ->
                    val admin: Player = command.sender()
    ...
    private fun canSuggestReward(CommandSender: CommandSender, edit: Boolean): Boolean {
        val player = CommandSender as? Player ?: return true

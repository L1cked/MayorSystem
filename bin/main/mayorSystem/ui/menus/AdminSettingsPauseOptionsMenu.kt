...
class AdminSettingsPauseOptionsMenu(plugin: MayorPlugin) : Menu(plugin) {
    ...
    override val title: Component = mm.deserialize("<gradient:#ff0000:#ff7a00>Settings: Pause Options</gradient>")
    ...
    override fun draw(player: Player, inv: Inventory) {
        ...
        inv.setItem(
            ...
            icon(
                Material.CLOCK,
                "<gold>Pause Options</gold>",
                listOf("<gray>Choose what the pause toggle affects.</gray>")
            )
        )
        ...
        for (i in options.indices) {
            ...
            val opt = options[i]
            val enabled = plugin.settings.pauseOptions.contains(opt)
